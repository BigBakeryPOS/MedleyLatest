    ...
    public class BSClass
    {
        ...
        public int UpdateUnitQty(int iGRId, string Unitname, int iProductId, string sBatchNo, String Expirydate, int iTotalqty, string sStatus, string Spalletsrefno,int Returnid)
        {
            ...
            else
            {
                ...
                {
                    string sQry = "update  tblUnit set TransGRId=0,ProductId='" + iProductId + "',Batchnumber='" + sBatchNo + "',Expirydate='" + Expirydate + "',Totalqty='" + iTotalqty + "',Status='" + sStatus + "',Palletsrefno='" + Spalletsrefno + "',Isempty=0,Returnid='" + Returnid + "' where Unitname='" + Unitname + "'";
                    iSuccess = dbObj.InlineExecuteNonQuery(sQry);
                }
            }
            return iSuccess;
        ...
        {
            int iSuccess = 0;
            string sQry = "UPDATE tblUnit SET TotalQty = " + sFinalbatchqty + " , isempty=" + status + ", TransGRId='', ProductId='',Status='', Palletsrefno='', Expirydate='1900-01-01' WHERE Unitname = '" + sUnitname + "'";// IF EXISTS (SELECT 1 FROM tblUnit WHERE Unitname =  '" + sUnitname + "' AND TotalQty = " + sFinalbatchqty + ") BEGIN UPDATE tblUnit SET Isempty = 1 WHERE Unitname = '" + sUnitname + "'; SELECT * FROM tblUnit WHERE Unitname = '" + sUnitname + "'; END ELSE BEGIN UPDATE tblUnit SET Isempty = 0 WHERE Unitname = '" + sUnitname + "'; SELECT * FROM tblUnit WHERE Unitname = '" + sUnitname + "'; END";
            // string sQry = "UPDATE tblUnit SET TotalQty = "+sFinalbatchqty+ " WHERE Unitname = '" + sUnitname + "'; IF EXISTS (SELECT 1 FROM tblUnit WHERE Unitname =  '" + sUnitname + "' AND TotalQty = " + sFinalbatchqty+ ") BEGIN UPDATE tblUnit SET Isempty = 1 WHERE Unitname = '" + sUnitname + "'; SELECT * FROM tblUnit WHERE Unitname = '" + sUnitname + "'; END ELSE BEGIN UPDATE tblUnit SET Isempty = 0 WHERE Unitname = '" + sUnitname + "'; SELECT * FROM tblUnit WHERE Unitname = '" + sUnitname + "'; END";
            iSuccess = dbObj.InlineExecuteNonQuery(sQry);
            return iSuccess;
        ...
        {
            DataSet ds = new DataSet();
            string sQry = "select * from tblGoodReceipts gr INNER JOIN tblProduct p on gr.ProductId=p.ProductID inner join TblPO po on po.Poid=gr.Poid order by GRId DESC";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        ...
        {
            DataSet ds = new DataSet();
            string sQry = "select top 1 * from tblTransGoodReceipts tg, tblUnit u  where tg.Unitname=u.Unitname and tg.Unitname ='" + Unitname + "' and tg.Finalbatchqty !=0 order by GRId desc";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        ...
        public DataSet checkProdid(string sBatchNo)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    sQry = "select * from tblgoodspicking g, tbltransGoodpicking gp,tblSO so, tblTransSupplier s inner join Suplier su on s.supplierid=su.supplierid where gp.Batchnumber='"+sBatchNo+ "' and unitname='"+unitname+"' and gp.prodid = '" + prodId + "' and g.id = gp.goodpickid and g.suppid = s.TransSupplierID and so.Soid=g.SoId and gp.Qty<>0";
                    ds = dbObj.InlineExecuteDataSet(sQry);
                }
        ...
        {
            DataSet ds = new DataSet();
            string sQry = "select tgp.GoodPickid,so.SOPrintno,gp.PickupDate,su.CompanyName,s.PersonName,s.Address,p.Productname,sum(tgp.Qty) as Qty,gp.PickupDate,tgp.Unitname,tr.ExpiryDate,tr.Batchnumber,so.Deliverydate,so.Refno from tblTransGoodPicking tgp inner join tblGoodsPicking gp on tgp.GoodPickid = gp.id inner join tblTransSupplier s on gp.SuppId = s.TransSupplierID inner join Suplier su on s.SupplierID = su.SupplierID inner join tblProduct p  on p.productid = tgp.ProdId inner join TblSO so on gp.soid = so.soid inner join tblunit u on u.Unitname = tgp.Unitname inner join tblGoodReceipts tr  on tr.Batchnumber = u.Batchnumber  where gp.SoId = '"+pickid+"' and tgp.Qty <> 0 group by gp.PickupDate,su.CompanyName,s.PersonName,p.Productname,gp.PickupDate,tgp.GoodPickid,so.SOPrintno,s.Address,tgp.Unitname,tr.ExpiryDate,tr.Batchnumber,so.Deliverydate,so.Refno";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        ...
        {
            DataSet ds = new DataSet();
            string sQry = "SELECT MAX(POPrintno+1) AS Printno FROM TblPO";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        #endregion
        ...
        {
            DataSet ds = new DataSet();
            string sQry = "SELECT MAX(SOPrintno+1) AS Printno FROM TblSO";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        #endregion
        ...
        {
            DataSet ds = new DataSet();
            string sQry = "SELECT MAX(ReturnPrintno+1) AS Printno FROM TblReturn";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        ...
        {
            DataSet ds = new DataSet();
            string sQry = "SELECT MAX(Returnid) AS Returnid FROM TblReturn";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        ...
        public DataSet POReportnew()
        {
            DataSet ds = new DataSet();
            string sQry = "SELECT tpo.*, p.Poid, p.POdatetime, s.CompanyName, s.ContactName, s.ContactNumber, s.ContactEmail, p.Amount,p.Status,p.POPrintno FROM Suplier s, TblPO p INNER JOIN tblTransPo tpo ON tpo.Poid = p.poid WHERE s.SupplierID = p.SupplierID ORDER BY p.Poid DESC";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        ...
        {
            DataSet ds = new DataSet();
            string sQry = "select p.*, u.unitname,u.Batchnumber,u.Expirydate,u.TotalQty,u.Status from tblunit u inner join tblProduct p on u.ProductId=p.ProductID where u.ProductId='" + Prodid + "'";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        ...
        {
            DataSet ds = new DataSet();
            string sQry = "select p.*, u.unitname,u.Batchnumber,u.Expirydate,u.TotalQty,u.Status from tblunit u inner join tblProduct p on u.ProductId=p.ProductID where u.ProductId='" + Prodid + "' and u.Batchnumber='" + Batchno + "'";
            ds = dbObj.InlineExecuteDataSet(sQry);
            return ds;
        }
        ...
        public DataSet SoReport_grid1(string sts)
        {
            DataSet ds = new DataSet();
